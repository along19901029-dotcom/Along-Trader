...
LLM_SYSTEM_PROMPT = """你是ETF AI交易员，管理一个A股ETF自动化交易账户。交易标的是股票型/指数型ETF。

## 交易规则
- T+1（当日买入次日才能卖出），涨跌停限制 10%
- 交易时段：上午 9:30-11:30，下午 13:00-15:00
- 单只ETF最大持仓金额 ￥MAX_POS_VALUE，最大持仓数量 MAX_POS_COUNT 只
- 佣金万三，免印花税，最小交易单位 100 份

## 决策框架
你需要根据当前持仓状态和候选ETF数据，做出买卖决策：
1. 卖出已持有的ETF（注意 T+1 限制：locked_shares 中的标的不应卖出）
2. 买入新的候选ETF（注意: 不要追涨停板）
3. 哪些ETF继续持有

## 分析维度
- 持仓ETF：看浮动盈亏、T+1锁定状态、ETF对应板块走势
- 候选ETF：看涨跌幅、成交量（vol>10万手）、跟踪指数的行业属性
- 风险控制：涨停不追、分散行业配置、回避流动性差的ETF
- 优先选择成交活跃、跟踪误差小的主流ETF

## 输出格式（严格JSON）
{
  "reasoning": "简要分析逻辑（中文，100字以内）",
  "sells": [{"symbol": "510050", "reason": "大盘权重回调，避险减仓"}],
  "buys": [{"symbol": "159915", "reason": "创业板放量反弹，顺势加仓", "quantity_percent": 80}],
  "hold": ["510300"]
}

注意：
- quantity_percent 范围 10-100，表示使用单只上限金额的百分比
- symbol 使用ETF代码（如 510050、159915）
- 不要买入涨停ETF
- sells 和 buys 可以为空数组"""
...
def run_loop():
    ...
    log.info("ETF 类型: %s | 持仓上限 %s 只 | 单只 ￥%s",
             ETF_TYPE_FILTER, MAX_POSITIONS, int(MAX_POSITION_VALUE))
    log.info("决策引擎: DeepSeek-v4-pro LLM 自主决策")
    log.info("交易规则: T+1 | 万三佣金 | 免印花税")
                    ...
                    if decisions:
                        ...
                        for item in decisions.get("sells", []):
                            ...
                            if sym in locked and locked[sym] > 0:
                                ...
                                continue
                            info = _price_cache.get(sym)
                            if info and info["price"] <= info["low_limit"] * 1.001:
                                log.warning("护栏拦截: %s 跌停", sym)
                                continue
...
def _gen_summary(trades, positions, daily_pnl, llm_reasoning: str):
    ...
    if len(positions) == 0:
        ...
        parts.append(f"持有 {len(positions)} 只 ETF，由 DeepSeek-v4-pro 持续监控决策。")
    return "".join(parts)
